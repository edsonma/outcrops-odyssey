...
increases, using the `Main` module will keep `Object` from being
...
of the file within the platform specific save enclave. `nil` will be
returned if the file is not found.

?> Use this function to store user-specific data. On most platforms, a
`__save_data__` directory will be created (console and other
locked-down platforms have a dedicated save enclave that we store and
retrieve from on your behalf). This `__save_data__` directory will not
be packaged with your game/will be automatically ignored.

?> See `samples/99_genre_board_game/03_word_game` for usage of `(read|write)_save_data`.

```
def tick args
  if Kernel.tick_count == 0
    # IMPORTANT: the root directory for the save enclave is handled internally.
    completed_levels = DR.read_save_data("completed_levels.txt")
    stats_total_playtime = DR.read_save_data("stats/play_time.txt")
    stats_total_deaths = DR.read_save_data("stats/deaths.txt")
  end
end
```
...
This function takes in two parameters. The first parameter is the file path and will be nested within the "save enclave". The second parameter is the string that will be written. The method ****overwrites**** whatever is currently in the file.
...
?> `read_file` and `write_file` should be used for managing game
related content (independent of the user that's currently playing the
game). Use this function for things like DLC and "global" game data
that persists regardless of the user that may be playing.
...
?> The DragonRuby Console can be shown using one of the following keys: `~`, `²`, `§`, `º`, `^` (or more generally, "the key next to `1` on the keyboard").
