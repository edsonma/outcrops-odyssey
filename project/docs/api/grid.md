...
Returns either `:bottom_left` (default) or `:center`. The initial origin
on boot can be set in `./mygame/metadata/game_metadata.txt`:

```
origin=bottom_left  # default
# origin=center
```
...
Returns the width component of the aspect ratio. For the default 16:9 mode
this is `16` in landscape and `9` in portrait. When a non-default
`aspect_mode` is active the value reflects the actual computed ratio.

## `aspect_ratio_h`

Returns the height component of the aspect ratio. For the default 16:9 mode
this is `9` in landscape and `16` in portrait. When a non-default
`aspect_mode` is active the value reflects the actual computed ratio.

## `aspect_size`

Returns the current `aspect_size` value (default `720`). Together with
`aspect_mode`, this determines the game's logical resolution.

### Aspect Mode and Size (`game_metadata.txt`)

The logical resolution of the game canvas is controlled by two settings in
`./mygame/metadata/game_metadata.txt`:

- `aspect_mode=0` **16:9** (default). `aspect_size` is the height seed;
  `logical_w = (aspect_size / 9) * 16`, `logical_h = aspect_size`.

  | `aspect_size` | Logical resolution |
  | ------------- | ------------------ |
  | 48            | 84×48              |
  | 72            | 128×72             |
  | 90            | 160×90             |
...
- `aspect_mode=1` **1:1**. Both width and height equal `aspect_size`.
...
Example `game_metadata.txt` for a 64×64 canvas:
...
?> For demonstration see: `./samples/99_genre_lowrez/resolution_64x64` (1:1)
and `./samples/99_genre_lowrez/nokia_3310_snake` (16:9 low-rez).
