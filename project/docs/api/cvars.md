...
|                                | `origin`                   | `bottom_left` or `center` (default is `bottom_left`)                         | Sets the grid origin on boot. `bottom_left` places `0, 0` at the bottom-left corner; `center` places `0, 0` at the center of the screen. Can also be changed at runtime via `Grid.origin_bottom_left!` / `Grid.origin_center!`. |
|                                | `aspect_mode`              | `0` or `1` (default is `0`)                                                  | Controls the aspect ratio of the logical canvas. `0` = 16:9, `1` = 1:1. See `aspect_size` for the resolution table. |
|                                | `aspect_size`              | Integer (default is `720`)                                                   | Sets the logical resolution seed. For `aspect_mode=0` (16:9): `logical_w = (aspect_size / 9) * 16`, `logical_h = aspect_size` (e.g. `48 -> 84×48`, `720 -> 1280×720`). For `aspect_mode=1` (1:1): both dimensions equal `aspect_size` (e.g. `64 -> 64×64`). |
