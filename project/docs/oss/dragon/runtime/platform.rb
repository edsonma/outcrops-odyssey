      ...
      def platform_mappings
        if !@platform_mappings
          baseline = {
            ...
            "iOS"        => [:mobile, :ios, :touch, :locked_down],
            "Android"    => [:mobile, :android, :touch, :locked_down],
            "Steam Deck" => [:steamdeck, :steam_deck, :steam, :steam_tenfoot],
            "PlayStation 5" => [:ps5, :console, :locked_down],
            "Nintendo Switch" => [:switch, :console, :locked_down],
            "Nintendo Switch 2" => [:switch2, :console, :locked_down],
          }
          ...
          if @is_steam_release
            ...
            if @is_steam_tenfoot
              baseline["Mac OS X"] << :steam_tenfoot
              baseline["macOS"] << :steam_tenfoot
              baseline["Windows"] << :steam_tenfoot
              baseline["Linux"] << :steam_tenfoot
            end
          end
