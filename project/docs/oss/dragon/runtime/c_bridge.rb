  ...
  * ERROR:
  ...
  S
      ...
      def windowevent_size_changed orientation_w, orientation_h,
                                   ...
                                   aspect_hash
        g = @args.grid
        ratio = allscreen_w_px.fdiv(allscreen_h_px)
        should_change_orientation = Grid.supported_orientations.include?(:landscape) &&
                                    Grid.supported_orientations.include?(:portrait) &&
                                    ((g.landscape? && ratio < 0.8) || (g.portrait? && ratio > 0.8))

        # indicates that orientation was changed
        if should_change_orientation
        ...
        g.aspect_mode           = aspect_hash[:aspect_mode]
        g.aspect_size           = aspect_hash[:aspect_size]
        g.__aspect_ratio_w__    = aspect_hash[:aspect_ratio_w]
        g.__aspect_ratio_h__    = aspect_hash[:aspect_ratio_h]
        ...
        if g.origin_name == :center
          g.render_origin_x = g.w / 2
          g.render_origin_y = g.h / 2
        else
          g.render_origin_x = 0
          g.render_origin_y = g.h
        end
