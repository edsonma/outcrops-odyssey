  ...
  class Runtime
    ...
    module Hotload
      ...
      def reload_if_needed file, force = false
        if file.end_with? ".rb"
          reload_ruby_file_if_needed file, force
        elsif file.end_with? ".hlsl"
          reload_shader_file_if_needed file, force
        end
      end

      def reload_shader_file_if_needed file, force = false
        @file_mtimes[file] ||= { current: @ffi_file.mtime(file), last: 0 }
        @file_mtimes[file].current = @ffi_file.mtime(file)
      ...
      def reload_ruby_file_if_needed file, force = false
