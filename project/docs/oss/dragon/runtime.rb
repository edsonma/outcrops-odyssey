...
module GTK
  ...
  class Runtime
    ...
    attr_reader :ffi_file, :ffi_mrb, :ffi_misc, :input_history, :recording, :console, :reserved_primitives,
                :ffi_draw, :args, :controller_config,
    ...
    def initialize(platform, production, vrmode, orientation_w, orientation_h, argv, binary_path, y_offset, orientation,
                   remote_hotload_enabled, a11y_enabled, is_origin_center, is_steam_tenfoot)
      @__sdl_tick_too_early_count__ = 0
      @__pending_log_entry_count__ = 0
      $gtk = self if $gtk.nil?
      $dr = self if $dr.nil?
      $runtime = self if $runtime.nil?
      @grid_origin_on_boot = is_origin_center ? :center : :bottom_left
      ...
      @console = Console.new
      @tests = Tests.new
      @args = Args.new self, @recording, orientation_w, orientation_h
      ...
      @is_steam_tenfoot = is_steam_tenfoot
      ...
      * INFO - If you need to find rogue puts statements do:
      ...
      disable_console! if platform?(:locked_down) && @production && !@remote_hotload_enabled
...
S
    ...
    def write_save_data file_name, text
      @ffi_file.write_save_data file_name, text
    end

    def read_save_data file_name
      @ffi_file.read_save_data file_name
    end
...
S
    ...
    def __reset_render_targets__
      ...
      @args.render_target_state.__data__.clear
    end
...
S
    ...
    def tick_controller_config
      ...
      should_tick_controller_config = false if platform?(:locked_down)
    ...
    def __sdl_tick__ sdl_tick
      ...
      render_targets_updated = @args.render_target_state.global_last_changed_at == Kernel.global_tick_count
    ...
    def pretty_print_exception_and_export! e
      ...
      if @production && platform?(:locked_down)
