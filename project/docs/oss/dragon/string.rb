...
class String
  ...
  def self.utf8_chars(str)
    ...
    while i < bytes.length
      ...
      chars << bytes[i, n].pack('C*')
      i += n
    end
    chars
  end

  def self.wrapped_lines_character_boundary string, length
    string.each_line.map do |line|
      output_lines = []
      line = line.rstrip
      multibyte = String.include_multibyte_char?(line)
      line_len  = multibyte ? utf8_chars(line).length : line.length
      ...
      if line_len <= length
        ...
        words.each do |word|
          word_len = multibyte ? utf8_chars(word).length : word.length
          ...
          if word_len > length
            ...
            current_line = ""
            current_len  = 0
            # char-split the oversized word; last partial chunk stays in current_line
            (multibyte ? utf8_chars(word) : word.split('')).each do |char|
              if current_len + 1 > length
                output_lines << current_line + "\n"
                current_line = char
                current_len  = 1
              else
          ...
          elsif current_len + 1 + word_len > length
            output_lines << current_line + "\n"
            current_line = word
            current_len  = word_len
          else
        ...
        (multibyte ? utf8_chars(line) : line.split('')).each do |char|
          if current_len + 1 > length
            output_lines << current_line + "\n"
            current_line = char
            current_len  = 1
  ...
  def self.wrapped_lines_word_boundary string, length
    string.each_line.map do |line|
      output_lines = []
      line = line.rstrip
  ...
  def self.wrapped_lines string, length
    if String.include_multibyte_char? string
      String.wrapped_lines_character_boundary string, length
    else
      String.wrapped_lines_word_boundary string, length
    end
  end
