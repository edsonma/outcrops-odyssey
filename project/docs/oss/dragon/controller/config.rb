  ...
  class Controller
    class Config
      ...
      def render_basics args, msg, fade=255
        ...
        args.outputs.primitives << [0, 0, Grid.w, Grid.h, 255, 255, 255, fade].solid
        args.outputs.primitives << [0, 0, Grid.w, Grid.h, 'dragonruby-controller.png', 0, fade, 255, 255, 255].sprite
        args.outputs.primitives << [Grid.w / 2, 700, joystickname, 2, 1, 0, 0, 0, fade].label
        args.outputs.primitives << [Grid.h / 2, 650, msg, 0, 1, 0, 0, 0, 255].label if !msg.empty?
      end
      ...
      def play_around args
        ...
        args.outputs.primitives << [Grid.w / 2, 90, '[ESCAPE]: Reconfigure, [SPACE]: Save this configuration', 0, 1, 0, 0, 0, 255].label
      ...
      def tick args
        ...
        args.outputs.primitives << [Grid.w / 2, 575, "Please press the #{part[2]}.", 0, 1, 0, 0, 0, 255].label
        render_part_highlight args, part, @part_alpha
        args.outputs.primitives << [Grid.w / 2, 90, '[ESCAPE]: Ignore controller, [BACKSPACE]: Go back one button, [SPACE]: Skip this button', 0, 1, 0, 0, 0, 255].label
        if !$gtk.production?
          args.outputs.primitives << [Grid.w / 2, 60, '(You can disable this screen with $gtk.disable_controller_config at the top of main.rb)', 0, 1, 0, 0, 0, 255].label
        end
