...
module GTK
  class Grid
    ...
    attr :aspect_mode, :aspect_size, :__aspect_ratio_w__, :__aspect_ratio_h__
    ...
    def initialize runtime, w, h
      @runtime = runtime
      @ffi_draw = runtime.ffi_draw
      @w = w
      @h = h
      if runtime.grid_origin_on_boot == :center
        __origin_center__!
      else
    ...
    def origin_bottom_left!(force: false)
      ...
      @ffi_draw.set_grid false
    end

    def set_aspect aspect_mode, aspect_size
      @ffi_draw.set_aspect(aspect_mode, aspect_size)
    end

    def origin_center!(force: false)
      __origin_center__!(force: force)
      @ffi_draw.set_grid true
    end
    ...
    def supported_orientations
      if !@supported_orientations
        orientation_cvar = Cvars["game_metadata.orientation"].value
        orientation_ios_cvar = Cvars["game_metadata.orientation_ios"].value
        orientation_android_cvar = Cvars["game_metadata.orientation_android"].value
        resolved_orientation_cvar = if @runtime.platform?(:ios) && orientation_ios_cvar.length != 0
                                      orientation_ios_cvar
                                    elsif @runtime.platform?(:android) && orientation_android_cvar.length != 0
                                      orientation_android_cvar
                                    else
                                      orientation_cvar
                                    end

        @supported_orientations = resolved_orientation_cvar.strip
                                                           .split(",")
                                                           .map(&:strip)
                                                           .map(&:to_sym)
      end
    ...
    def aspect_ratio_w
      landscape? ? @__aspect_ratio_w__ : @__aspect_ratio_h__
    end

    def aspect_ratio_h
      landscape? ? @__aspect_ratio_h__ : @__aspect_ratio_w__
    end
    ...
    def __origin_center__!(force: false)
      if !force
    ...
    class << self
      ...
      def aspect_size = $grid.aspect_size
      def set_aspect(aspect_mode, aspect_size) = $grid.set_aspect(aspect_mode, aspect_size)
      def x = $grid.x
      def x_px = $grid.x_px
      def y = $grid.y
      def y_px = $grid.y_px
