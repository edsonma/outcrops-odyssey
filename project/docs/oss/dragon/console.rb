...
module GTK
  class Console
    ...
    def console_text_width
      if Grid.landscape?
        (1280 - 20).idiv(font_style.letter_size.w)
      else
        (720 - 20).idiv(font_style.letter_size.w)
      end
    end
    ...
    def toggle
      if !@visible
        DR.start_text_input
        Grid.set_aspect(0, 720)
        @cursor_visibilty_before_show = $gtk.cursor_shown?
        $gtk.show_cursor
      else
        DR.stop_text_input
        @original_aspect_mode ||= Cvars["game_metadata.aspect_mode"].value
        @original_aspect_size ||= Cvars["game_metadata.aspect_size"].value
        Grid.set_aspect(@original_aspect_mode, @original_aspect_size)
        if @cursor_visibilty_before_show == false
          $gtk.hide_cursor
        end
      end
...
S
    ...
    def calc args
      # need this additional check in case there is already
      # an exception, but then another one is thrown while
      # the console is open
      if Grid.aspect_size != 720
        Grid.set_aspect(0, 720)
      end
