...
module GTK
  ...
  module WatchLabels
    def watch o, label_style: nil, background_style: nil, font: nil, size_px: nil
      ...
      label_style = { **label_style, font: font, size_px: size_px }
    ...
    def self.prefab sender:, string:, label_style: nil, background_style: nil
      label_style ||= { size_px: 20, anchor_y: 0 }
      label_style = { **label_style }

      size_px = label_style[:size_px] || 20
      size_enum = label_style[:size_enum]
      if size_enum
        size_px = (size_enum * 0.5 + 22)
      end
      label_style.delete :size_enum
      label_style[:size_px] = size_px
      ...
      w, h = $gtk.calcstringbox string, size_px: size_px
      longest_w = w
      longest_h = h
      ...
      while idx < l
        if idx < @@watch_label_column
          longest_w, longest_h = $gtk.calcstringbox @@watch_label_longest_text[idx][:string], size_px: @@watch_label_longest_text[idx][:style][:size_px]
...
module GTK
  class Outputs   # Each Outputs is a single render pass to a render target (or the window framebuffer).
    ...
    def initialize(args:, height: nil, width: nil, target: nil, background_color_override: nil)
      @target = nil
      @target = target.to_s if target
      @width  = width  || $grid.w
      @height = height || $grid.h
      @args = args
      ...
      @background_color = background_color_override || default_background_color
...
S
    ...
    def render_target_state
      @args.render_target_state
    end

    alias_method :rt_state, :render_target_state

    def render_targets
      raise "* ERROR: ~Outputs#render_targets~ renamed to ~Outputs#render_target_state~"
    end

    def rts
      raise "* ERROR: ~Outputs#rts~ renamed to ~Outputs#rt_state~"
    end
  end
  ...
  module ShaderProps
    ...
    def shader= value
      if value && value[:path] && !DR.shader_metadata[value[:path]] && !DR.paused?
        raise <<-S
* ERROR: Shader must be required before it can be used.

Shaders must be required (just like ruby files) before you can use them.
...
S
    ...
    def shader_disabled?
      ...
      return true if !@shader[:path]
      return true if @shader[:path].strip == ""
      return false
    end
  end
  ...
  class RenderTargetOutputs < Outputs
    ...
    def initialize(args:,
                   ...
                   width: nil,
                   target: nil,
                   background_color_override: nil,
                   render_target_data: nil)
      super(args: args,
            height: height,
            width: width,
            target: target,
            background_color_override: background_color_override)
      @rt_data = render_target_data
      @transient = true
    end
